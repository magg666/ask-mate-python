...
# add sorting, converted timestamp
@app.route('/')
@app.route('/list')
def route_questions_list():
	"""
	Function supports main and /list page
	:return questions list on page:
	"""
	all_data = data_manager.sort_questions()
...
def route_question_view(question_id):
	...
	if request.method == 'GET':
		...
		connection.update_data_by_id(
			QUESTION_FILE_PATH, QUESTION_HEADER, question_id, single_question)
		...
		connection.update_data_by_id(QUESTION_FILE_PATH, QUESTION_HEADER, question_id,
									 single_question)
		return render_template('question_page.html',
							   single_question=single_question,
							   all_answers=all_answers)
...
def edit_question(question_id):
	...
	if request.method == 'GET':
		...
		update_question = data_manager.create_question(question_submission_time, question_id, question_view, question_vote)
		connection.update_data_by_id(QUESTION_FILE_PATH, QUESTION_HEADER, question_id,
									 update_question)
		return redirect('/')
